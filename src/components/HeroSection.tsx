...
const HeroSection = () => {
  ...
  return <section ref={heroRef} className="section-animate relative min-h-screen flex items-center justify-center overflow-hidden bg-hero">
      ...
      <div className="absolute inset-0 bg-mesh transition-opacity duration-700" style={{
      ...
      <div className="absolute inset-0">
        {/* Smaller subtle orbs */}
        <div className="absolute w-64 h-64 bg-gradient-to-r from-primary/10 to-secondary/5 rounded-full blur-3xl animate-float transition-transform duration-1000" style={{
        top: '15%',
        left: '10%',
        transform: `translate(${mousePosition.x * 20}px, ${mousePosition.y * 15}px)`
      }} />
        
        <div className="absolute w-48 h-48 bg-gradient-to-r from-accent/8 to-primary/5 rounded-full blur-3xl animate-float animation-delay-1000 transition-transform duration-1000" style={{
        bottom: '20%',
        right: '15%',
      ...
      {/* Main Content */}
      <div className={`relative z-10 text-center px-6 max-w-7xl mx-auto transition-all duration-1500 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        ...
        <div className="space-y-2 mb-12 relative">
          ...
          }} className="block text-7xl md:text-8xl font-black animate-slide-up tracking-tighter text-amber-300 lg:text-9xl">
              MOV
            </span>
        ...
        <div className="flex flex-col lg:flex-row gap-8 justify-center items-center animate-slide-up animation-delay-1200 mb-20">
          <Button className="btn-hero group text-xl px-12 py-6 relative overflow-hidden glow-border energy-pulse" onClick={handleCTAClick} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            ...
            <div className={`absolute inset-0 bg-gradient-to-r from-secondary to-primary transition-all duration-300 ${isHovered ? 'opacity-20 scale-105' : 'opacity-0 scale-100'}`} />
          </Button>
          
          <div className="flex items-center gap-4">
            <Button variant="outline" className="group px-8 py-6 bg-background hover:bg-primary/10 border-2 border-primary/30 hover:border-primary/50 text-foreground hover:text-foreground transition-all duration-300" onClick={handleVideoPlay}>
              <Play className="mr-3 w-5 h-5 group-hover:scale-110 transition-transform" />
              <span className="font-semibold">Ver Demo</span>
            </Button>
            
            <Button variant="ghost" className="px-8 py-6 text-foreground hover:text-primary hover:bg-primary/5 transition-all duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 animate-slide-up animation-delay-1500">
          ...
          icon: Sparkles
        }].map((stat, index) => <MagneticNumber key={index} value={stat.value} label={stat.label} icon={stat.icon} delay={index * 200} />)}
        </div>

        {/* Interactive Mouse Follower */}
        <div className="fixed w-4 h-4 bg-primary/30 rounded-full pointer-events-none z-50 transition-transform duration-100" style={{
