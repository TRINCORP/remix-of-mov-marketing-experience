    ...
    >
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="text-center mb-20 md:mb-32">
          ...
          <h2 
            className={`text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black mb-8 leading-[0.9] transition-all duration-1000 ${
        ...
        {/* Differentials Grid - Interactive Cards */}
        <div className="mb-20 md:mb-32">
          <h3 
            className={`text-3xl md:text-4xl font-bold text-center mb-12 md:mb-16 transition-all duration-1000 ${
              isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
            }`}
            ...
            {differentials.map((item, index) => {
              ...
              return (
                <div
                  ...
                  style={{ transitionDelay: `${600 + item.delay}ms` }}
                  onMouseEnter={() => setActiveCard(index)}
                  onMouseLeave={() => setActiveCard(null)}
                >
                  {/* Glow effect */}
                  <div 
                    className={`absolute inset-0 bg-gradient-to-br ${item.gradient} rounded-3xl blur-2xl transition-opacity duration-500 ${
                      isActive ? 'opacity-20' : 'opacity-0'
                    }`}
                  />
                  
                  {/* Card */}
                  <div className={`relative h-full bg-muted/20 backdrop-blur-sm rounded-3xl p-8 md:p-10 border transition-all duration-500 ${
                    isActive 
                      ? 'border-primary/50 scale-[1.02] shadow-2xl shadow-primary/10' 
                      : 'border-border/30 hover:border-border/50'
                  }`}>
                    {/* Icon */}
                    <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-all duration-500 bg-gradient-to-br ${item.gradient} ${
                    ...
                    {/* Decorative corner */}
                    <div className={`absolute top-6 right-6 w-20 h-20 transition-opacity duration-500 ${
